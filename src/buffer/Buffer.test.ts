...
describe('Buffer', () => {
  it('should expose the size and position', () => {
    const buffer = new Buffer({ width: 10, height: 5, x: 2, y: 3, z: 0 })
    expect(buffer.width).toBe(10)
    expect(buffer.height).toBe(5)
    expect(buffer.x).toBe(2)
    expect(buffer.y).toBe(3)
    expect(buffer.z).toBe(0)
  })

  it('should allow to set and get cells', () => {
    const buffer = new Buffer({ width: 5, height: 5, x: 2, y: 3, z: 0 })
    buffer.set(2, 2, 'A')
  ...
  describe('.entries', () => {
    it('should have no entries by default', () => {
      const buffer = new Buffer({ width: 10, height: 5, x: 0, y: 0, z: 0 })
    ...
    describe('flushing', () => {
      it('should return entries for all changed cells', () => {
        const buffer = new Buffer({ width: 5, height: 5, x: 0, y: 0, z: 0 })
        buffer.set(1, 1, null)
        expect(buffer.entries.length).toBe(1)
  ...
  describe('.debug', () => {
    it('should return a string representation of the buffer', () => {
      const buffer = new Buffer({ width: 3, height: 2, x: 0, y: 0, z: 0 })
      buffer.set(0, 0, 'A')
      buffer.set(1, 0, 'B')
      buffer.set(2, 0, 'C')
      buffer.set(0, 1, null)
